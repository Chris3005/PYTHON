    ...
    def wektorowanie(folder, metoda="kNN", min = 5, max = 0.5, słowa_do_usunięcia = False):
        ...
        if metoda == "LR":
            if słowa_do_usunięcia == True:
                ...
                wektoryzator_test = CountVectorizer(min_df = min, max_df = max, stop_words = None).fit(teksty_testowe) 
...
def main(): 
    ...
    klasyfikator_tekstów = KlasyfikatorTekstów("small_text_files", "LR")
