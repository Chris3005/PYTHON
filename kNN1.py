...
class PodziałDanych:
    ...
    def wektorowanie(folder, metoda="kNN", min = 0, max = 1.0, słowa_do_usunięcia = False):
        ...
        if metoda == "LR":
            if słowa_do_usunięcia == True:
                wektoryzator_tren = CountVectorizer(min_df = min, max_df = max, stop_words = 'english').fit(teksty_uczace)  
                wektoryzator_test = CountVectorizer(min_df = min, max_df = max, stop_words = 'english').fit(teksty_testowe)  
            else:
                wektoryzator_tren = CountVectorizer(min_df = min, max_df = max, stop_words = None).fit(teksty_uczace) 
                wektoryzator_test = CountVectorizer(min_df = min, max_df = max, stop_words = 'english').fit(teksty_testowe) 
            print("Utworzono wektoryzatory 1 i 2...") 
            X_treningowe = wektoryzator_tren.transform(teksty_uczace)
            print("Utworzono X_treningowe.")
            X_testowe = wektoryzator_test.transform(teksty_testowe)
            ...
            print("Utworzono X_treningowe.")
            X_testowe = wektoryzator.fit_transform(teksty_testowe)
            print("Utworzono X_testowe.")
    ...
    def tworzenie_klasyfikatora(self, liczba_sąsiadów = 1):
        ...
        if self.metoda == "LR":
            ...
            try:
                ...
                wynik1 = accuracy_score(self.y_testowe, przewidywane)
...
class Klasyfikator_Tekstów_RęcznyKNN(PodziałDanych):
    ...
    def __init__(self, folder):
        ...
        tren = "/treningowe"
        test = "/testowe"
        self.dzielenie_danych("small_text_files")

    def macierzowa_reprezentacja_danych():
        """Metoda służy do reprezentowania danych tekstowych w "workach danych"""
 
    


def main(): 
    hihi = PodziałDanych.wektorowanie("small_text_files")
    print("main: tworzenie obiektu KlasyfikatorTekstów...")
    klasyfikator_tekstów = KlasyfikatorTekstów("small_text_files", "kNN")
    print("main: koniec konstruktora KlasyfikatorTekstów, wynik:")
    wynik = klasyfikator_tekstów.tworzenie_klasyfikatora(1)
